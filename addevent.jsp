  ...
  <tbody>
    <tr>
      ...
      <td align="center" valign="top" class="subtitle"><p class="title">&nbsp;</p>
        ...
        <form name="form1" method="post" action="addevent.jsp" onSubmit='return formValidator()' >
          <table width="310" height="273" border="1">
            <tr bgcolor="#BF2B1F">
              <td height="27" colspan="2"><div align="center" class="style4">Add new events</div></td>
            </tr>
            <tr class="mm">
		...
		if(evnt!=null)
		{
        	try
        	{
                	rs = obj.select("SELECT 'event'+CONVERT(varchar, ISNULL(MAX(CONVERT(int, SUBSTRING(event_id, 6, LEN(event_id))) + 1), 1)) AS Head FROM Events WHERE (SUBSTRING(event_id, 1, 5) = 'event')");
                	if(rs.next())
                	{
						evntid=rs.getString(1);
               		}
					obj.idu("INSERT INTO Events VALUES('"+evntid+"','"+evnt+"','"+ty+"','"+cat+"',"+p1+","+p2+","+p3+","+ppl+")");
					out.println("New event has been successfully added");
					out.println("<br><br>");
