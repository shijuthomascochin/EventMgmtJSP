  ...
  <tbody>
    <tr>
      ...
      <td align="center" valign="top" class="subtitle"><p class="title">&nbsp;</p>
        ...
        <form name="form1" method="post" action="addresult.jsp" onSubmit='return formValidator()' >
          <table width="347" height="160" border="1">
            ...
            <tr class="mm">
              ...
              <td width="176"><select name="d1" id="d1" onChange="form1.submit();">
			  <option>Select an event</option>
			<%
				String evnt=request.getParameter("d1");
				String pt=request.getParameter("d2");
				String pnt=request.getParameter("d3");
            ...
            <tr class="mm">
			 ...
			 <%
			 	if(evnt!=null)
				{
					rs = obj.select("SELECT [chest_no] FROM [EventParticipants] WHERE [event_id]='"+evnt+"'");
					while(rs.next())
					{
						String sss = rs.getString(1);
					%>
						<option <%if(pt!=null && pt.equals(sss)) {%> selected="selected" <%}%> ><%=sss%></option>
					<%}
				}
			%>
             </select>
		...
		if(btn!=null && !evnt.equals("Select an event"))
		{
        	try
        	{
                rs = obj.select("SELECT [event_id],[pt_no] FROM [EventResults] WHERE [event_id]='"+evnt+"' AND [pt_no]="+pt+"");
                	if(rs.next())
                	{
						out.println("Result has already been entered for this event for this participant");
						out.println("<br><br>");
						out.println("Event id: "+evnt);
						out.println("<br>");
						out.println("Participant id: "+pt);
               		}
					else
					{
						String tid="",totpoint="";
						rs=obj.select("select team_id from Participant where chest_no="+pt+"");
						if(rs.next())
						{
							tid=rs.getString(1);
						}
						obj.idu("INSERT INTO [EventResults] VALUES('"+evnt+"',"+pt+","+pnt+")");
						...
						out.println("Result has been successfully added");
						out.println("<br><br>");
						out.println("Event id: "+evnt);
						out.println("<br>");
						out.println("Participant id: "+pt);
		...
		else if(evnt!=null && evnt.equals("Select an event"))
		{
			out.println("Please select an event");
